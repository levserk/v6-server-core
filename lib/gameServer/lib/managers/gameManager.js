...
module.exports = class GameManager extends Manager {
    ...
    onUserTimeout(engine, room, timeout) {
        ...
        if (room.turnStartTime !== timeout.turnStartTime) {
            ...
            return this.server.getUserSocket(room.game, timeout.userId).then((socket) => {
                if (socket) {
                    timeout.online = true;
                } else {
    ...
    sendUserEvent(room, userId, event) {
        return this.server.sendToUser(room.game, userId, {
            module: 'game_manager',
            type: 'event',
            data: event
        });
    ...
    sendRoundEnd(room, result, players) {
        ...
        return co(function* () {
            ...
            for (let playerId of room.players) {
                let socket = yield self.server.getUserSocket(game, playerId);
                if (!socket) {
    ...
    sendError(game, userId, error) {
        return this.server.sendToUser(game, userId, {
            module: 'game_manager',
            type: 'error',
            data: error
        });
