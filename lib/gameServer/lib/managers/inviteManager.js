...
module.exports = class InviteManager extends Manager {
    ...
    init() {
        ...
        return co(function* () {
            ...
            self.dispatcher.on(`game.user_message.invite_manager`, self.onNewMessage.bind(self));
            self.dispatcher.on(`system.close_room`, (game, roomId) => {
            ...
            self.dispatcher.on(`system.leave_user_room`, (game, userId, roomId) => {
    ...
    onInvite(user, target, game, invite) {
        ...
        return co(function* () {
            ...
            yield self.dispatcher.emit(`system.send_to_socket`, socket, JSON.stringify({
    ...
    onInviteCancel(user, target, game, invite) {
        ...
        return co(function*() {
            ...
            return self.dispatcher.emit(`system.send_to_socket`, socket, JSON.stringify({
                module: "invite_manager",
                type: "cancel",
                data: invite
            }));
    ...
    onInviteAccepted(user, targetId, game, invite) {
        ...
        return co(function* () {
            ...
            if (!targetSocket || !userSocket) {
                if (targetSocket) {
                    yield self.dispatcher.emit(`system.send_to_socket`, targetSocket, JSON.stringify({
                        module: "invite_manager",
                        type: "reject",
                        data: invite
                    }));
    ...
    onPlayRandom(user, game, data) {
        ...
        return co(function* () {
            ...
            if (waitingId) {
                ...
                yield self.dispatcher.emit(`system.send_to_sockets`, game, JSON.stringify({
                    module: "invite_manager",
                    type: "random_wait",
                    data: sendData
                }));
    ...
    createRoom(game, socketId, ownerId, players, inviteData, type) {
        ...
        return co(function* () {
            ...
            yield self.dispatcher.emit(`system.send_to_sockets`, game, {
                module: 'server',
                type: 'new_game',
                data: room.getInfo()
            });
        ...
        return co(function* () {
            ...
            if (userRoom.role === ROLE_PLAYER && userRoom.type === Room.TYPE_MULTY) {
                ...
                if (leaved) {
                    ...
                    yield self.dispatcher.emit(`system.user_leaved`, game, userId, userRoom);
                }
            }
        });
    }
