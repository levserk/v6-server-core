...
module.exports = class ChatManager extends Manager {
    ...
    init() {
        ...
        return co(function* (){
                    ...
                    yield self.subscribe(`user_send_chat_manager`, self.onMessage.bind(self));
                    return true;
                })
            .then(() => {
                self.isRunning = true;
    ...
    onMessage(message) {
        let user = message.sender, game = message.game, type = message.type, data = message.data;
        log(`onMessage`, `user: ${user.userId}, game: ${game}, type: ${type}, data: ${data}`);
        switch (type){
            case 'message':
                ...
                return this.sendMessage(user, game, data.text, data.target, data.admin);
            case 'load':
                return this.loadMessages(user, game, data.count, data.time, data.target || this.default, data.type);
            case 'ban':
                if (user.isAdmin || this.server.conf.mode == 'debug' || this.server.conf.mode == 'develop') {
                    return this.banUser(data.userId, data.days, data.reason);
                } else {
                    return Promise.resolve(true);
                }
            case 'delete':
                if (user.isAdmin || this.server.conf.mode == 'debug' || this.server.conf.mode == 'develop') {
                    return this.deleteMessage(game, data.time);
                } else {
                    return Promise.resolve(true);
                }
        }
    ...
    loadMessages(user, game, count, time, target, type) {
        count = +count;
        time = +time;
        ...
        return co(function* (){
            let messages = [];
            let socket = yield self.server.userManager.getUserSocket(game, user.userId);
            yield self.server.userManager.sendToSocket(socket,
                `{ "module": "chat_manager","type": "load","data": ${JSON.stringify(messages)} }`)
        });
