...
module.exports = class GameManager extends Manager {
    ...
    onMessage (message){
        ...
        return co(function* (){
            ...
            //load room
            if (userRoom.role == 'player' || type == 'leave' || type == 'leaved'){
                room = yield self.loadRoom(game, userRoom.roomId);
                if (!room){
                    throw new Error(`no room ${game}, ${userRoom.roomId}`)
                }
            }
            ...
            return self.server.inviteManager.closeRoom(game, userRoom.roomId);
        })
    };
    ...
    onSpectatorLeave(engine, room, user){
        ...
        return co(function* (){
            ...
            yield self.server.inviteManager.leaveRoom(userId, game, room.id, true);
        })
    }
