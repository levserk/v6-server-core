    ...
    onMessage (message){
        ...
        return co(function* (){
            switch (message.type){
                ...
                case "close_room": yield self.closeRoom(message.game, message.data.roomId); break;
                case "user_leave_room": yield self.leaveUserRoom(message.data.userId, message.game, message.data.roomId); break;
            }
            yield self.memory.del(`invites_current:${game}`);
