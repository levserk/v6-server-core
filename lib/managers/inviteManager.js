...
module.exports = class InviteManager extends Manager {
    ...
    leaveCurrentUserRoom(userId, game) {
        ...
        return co(function* (){
            ...
            if (userRoom.role === ROLE_PLAYER && userRoom.type === TYPE_MULTI) {
                ...
                return self.leaveRoom(userId, game, roomData.roomId);
            }
        });
    }
    ...
    leaveRoom(userId, game, roomId, silent) {
        let self = this;
        return co(function* () {
            let userRoom = yield self.getUserRoom(userId, game);
            if (!userRoom) {
            ...
            if (userRoom.roomId !== roomId){
                ...
                yield self.delUserRoom(userId, game);
                if (!silent){
                    // not game manager send closing room
                    yield this.server.gameManager.onNewMessage({
                        sender: userId,
                        game: game,
                        userRoom: userRoom,
                        type: 'leaved'
