...
module.exports = class SocketManager extends Manager {
    ...
    registerServer() {
        ...
        return co(function* () {
            ...
            if (check) throw Error (`ws server on port: ${self.conf.port} already run, or closed incorrectly!`);
...
function createWebServer(conf){
    return new Promise((res, rej) => {
        ...
        if (server && !server.listen) throw Error("Server in options must be http or https server");
        if (!conf.https) server = require("http").createServer(response);
        else {
            if (!conf.httpsKey || !conf.httpsCert) throw Error("Check https key and certificate in options");
            let httpsObj = {
