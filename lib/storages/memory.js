...
module.exports = class Memory {
    ...
    init() {
        ...
        return co(function* (){
            ...
            if (self.conf.clear) yield self.clear();
    ...
    getKeys(pattern) {
        if (!key || typeof key != "string") {
            throw Error(`wrong parameters to getKeys in memory, pattern: ${pattern}`);
        }
    ...
    setMembers(key) {
        if (!key || typeof key != "string") {
            throw Error(`wrong parameters to smembers in memory, key: ${key}, ${value}`);
        }
    ...
    hashRemove(key, property) {
        if (!key || !property || typeof key != "string") {
            throw Error(`wrong parameters to hmset in memory, key: ${key}, ${value}, ${property}`);
        }
