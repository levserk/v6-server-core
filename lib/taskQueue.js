...
let co = require('co');
let Task = require('./instances/Task.js');
...
module.exports = class TaskQueue {
     ...
     */
    testRedis() {
        let self = this, key = 'test_redis_' + Math.random(), data = 'test_data', cancelTimeout;
        return new Promise((res, rej) => {
            self.subscribe(key, {
                onTask(p, _task) {
                    clearTimeout(cancelTimeout);
                    log(`testRedis`, `test redis complete, data: ${_task.data}, equal: ${_task.data == data} `, 4);
                    res(true);
